...
PreDock_scores=($(awk '{print $25}' ./${results_folder}_scores.sc))
echo "PreDock"
echo ${PreDock_scores[@]}
rmsd=($(awk '{print $36}' ./${results_folder}_scores.sc))
echo "RMSD"
echo ${rmsd[@]}
total_energy_final=($(awk '{print $38}' ./${results_folder}_scores.sc))
echo "total_energy_final"
echo ${total_energy_final[@]}
total_energy_starting=($(awk '{print $39}' ./${results_folder}_scores.sc))
echo "Total Energy Starting"
echo ${total_energy_starting[@]}
environment=($(awk '{print $41}' ./${results_folder}_scores.sc))
echo "environment"
echo ${environment[@]}
Pose_names=($(awk '{print $42}' ./${results_folder}_scores.sc))
echo "Pose_names"
echo ${Pose_names[@]}
...
printf "Number;Pose;PreDock;PostDock;PostRelax1;RMSD;TotalEnergy_Starting;TotalEnergy_Final;Environemnt\n" > ${results_folder}_results.csv

for ((i = 0 ; i < ${#Pose_names[@]} ; i++ ));
do
printf "${i};${Pose_names[$i]};${PreDock_scores[$i]};${PostDock_scores[$i]};${PostRelax1_scores[$i]};${rmsd[$i]};${total_energy_starting[$i]};${total_energy_final[$i]};${environment[$i]}\n" >> ${results_folder}_results.csv
done

# sort 20 best results by PostRelax1
csvsort -c PostRelax1 ${results_folder}_results.csv | head -n 2 > best_${results_folder}1.csv
...
#Create Plots in R
Rscript ../hist.r "${results_folder}_results.csv" "${results_folder}_i_score.png" "${results_folder}_i_score"

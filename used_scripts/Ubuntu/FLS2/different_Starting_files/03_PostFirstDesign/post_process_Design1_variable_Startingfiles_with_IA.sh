...
#Which Number for Starting-File?
echo which number for $m

read number

starting_file=${starting_file}${number}".pdb"
...
echo "Environment"
echo ${environment[@]}
Pose_names=($(awk '{print $87}' ./${results_folder}_scores.sc))
echo "Pose_names"
echo ${Pose_names[@]}
...
printf "Pose;dG_preDesign;dG_postDesign;unsatHbond_postDesign;I_Score;Packstat;sc_value_post;Mutations;Charge;SASA_Int;RMSD;Total_Energy_Starting;Total_Energy_Final;Environmen\n" > ${results_folder}_results.csv
	
for ((i = 0 ; i < ${#Pose_names[@]} ; i++ ));
do
printf "${Pose_names[$i]};${dG_preDesign[$i]};${dG_postDesign[$i]};${i_score[$i]};${d_unsatHbond[$i]};${Packstat[$i]};${sc_value_post[$i]};${mutations[$i]};${charge[$i]};${SASA[$i]};${rmsd[$i]};${total_energy_starting[$i]};${total_energy_final[$i]};${environment[$i]}\n" >> ${results_folder}_results.csv
done
